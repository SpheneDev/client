...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    public Task<(SpheneCharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
    ...
    public Dictionary<string, string> McdfExtractFiles(SpheneCharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        SpheneCharaFileHeader.AdvanceReaderToData(reader);
