...
public sealed class PerformanceCollectorService : IHostedService
{
    ...
    public PerformanceCollectorService(ILogger<PerformanceCollectorService> logger, SpheneConfigService SpheneConfigService)
    {
        _logger = logger;
        _SpheneConfigService = SpheneConfigService;
    }

    public T LogPerformance<T>(object sender, SpheneInterpolatedStringHandler counterName, Func<T> func, int maxEntries = 10000)
    {
        if (!_SpheneConfigService.Current.LogPerformance) return func.Invoke();
    ...
    public void LogPerformance(object sender, SpheneInterpolatedStringHandler counterName, Action act, int maxEntries = 10000)
    {
        if (!_SpheneConfigService.Current.LogPerformance) { act.Invoke(); return; }
    ...
    {
        if (!_SpheneConfigService.Current.LogPerformance)
        {
