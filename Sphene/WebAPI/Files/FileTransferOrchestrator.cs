...
public class FileTransferOrchestrator : DisposableMediatorSubscriberBase
{
    ...
    public FileTransferOrchestrator(ILogger<FileTransferOrchestrator> logger, SpheneConfigService SpheneConfig,
        SpheneMediator mediator, TokenProvider tokenProvider, HttpClient httpClient) : base(logger, mediator)
    {
        _SpheneConfig = SpheneConfig;
        _tokenProvider = tokenProvider;
        _httpClient = httpClient;
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        _httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("MareSynchronos", ver!.Major + "." + ver!.Minor + "." + ver!.Build));

        _availableDownloadSlots = SpheneConfig.Current.ParallelDownloads;
    ...
    {
        var limit = _SpheneConfig.Current.DownloadSpeedLimitInBytes;
        if (limit <= 0) return 0;
        limit = _SpheneConfig.Current.DownloadSpeedType switch
        {
            SpheneConfiguration.Models.DownloadSpeeds.Bps => limit,
            SpheneConfiguration.Models.DownloadSpeeds.KBps => limit * 1024,
            SpheneConfiguration.Models.DownloadSpeeds.MBps => limit * 1024 * 1024,
            _ => limit,
        };
